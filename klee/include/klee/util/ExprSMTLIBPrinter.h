...
public:
  ...
  /// Lewis: add a function to print a given expression in SMT2 format
  void printMemoryExpression(ref<Expr> e) {
    /* reset all data structures, each time printer is called */
    reset();
    setAbbreviationMode(klee::ExprSMTLIBPrinter::ABBR_NONE);
    /* "scan" must be called before printing the array devclaration */
    scan(e);
    printOptions();
    printSetLogic();
