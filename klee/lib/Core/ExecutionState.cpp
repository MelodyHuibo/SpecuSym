...
ExecutionState::ExecutionState(const ExecutionState& state):
  ...
  stateType(SYMBOLIC),
  id(counter++),
  parState((ExecutionState*)&state), 
  addrs(state.addrs),
  objNames(state.objNames),
  pcInfos(state.pcInfos),
  marks(state.marks),
  rbc(state.rbc),
  continueFlag(state.continueFlag),
  rsp(state.rsp),
  old_rsp(state.old_rsp),
  regObj(false),
  simpleModeFlag(state.simpleModeFlag),
  nestedCnt(state.nestedCnt),
  analyzeNormalFlag(state.analyzeNormalFlag)
{
  for (unsigned int i=0; i<symbolics.size(); i++)
