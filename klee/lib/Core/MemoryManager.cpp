...
MemoryObject *MemoryManager::allocate(uint64_t size, bool isLocal,
  ...
  if (xAddr > 0 && xAddr != 0xdeadbeef) {
//    fprintf(stderr, "    Rewrite address to [0x%X]\n", xAddr);
    int64_t shift = 0;
    shift = address - xAddr;
    res = new MemoryObject(xAddr, size, isLocal, isGlobal, false, allocSite, this);
    res->shift = shift;
  } else {
    res = new MemoryObject(address, size, isLocal, isGlobal, false, allocSite, this);
    res->shift = 0;
    res->isRegObj = xAddr == 0xdeadbeef ? true: false;
  }
