...
// sjguo: use libconfig to read cfg file
#include <libconfig.h++>
...
public:
  ...
  // map memory layout in klee to realistic
  std::map<std::string, unsigned> memoryMapFromFile;

  // eliminate memory access caused by klee instrumentation
  std::vector<unsigned> kleeMemAccessLine;

  libconfig::Config cacheConfig;
  // libconfig::Config memmapConfig;
  static unsigned setNum;
  static unsigned lineSize;
  static unsigned nway;
  static unsigned replace;
  static uint64_t stackBase;

  std::map<std::pair<unsigned, unsigned >, ref<Expr> > setPairCache;
  std::map<std::pair<unsigned, unsigned >, ref<Expr> > tagPairCache;
  std::map<std::pair<unsigned, unsigned >, ref<Expr> > setagPairCache;
  std::map<unsigned, ref<Expr> > setCache;
  std::map<unsigned, ref<Expr> > tagCache;
  std::map<unsigned, ref<Expr> > setagCache;

  std::map<std::pair<unsigned, unsigned>, Solver::Validity> cmpResultCache;
  std::map<std::pair<unsigned, unsigned>, ref<Expr> > uCnstrCache;

  // read cache configuration from file
  void loadConfigFile(std::string path);
  // read memory map from file
  void loadMemMapFile(std::string path);
  // read klee memory access from file
  void loadKleeMemAccessFile(std::string path);

  static int log_base2(int n);

  ref<Expr> getSet(ref<Expr> addr);
  ref<Expr> getTag(ref<Expr> addr);
  ref<Expr> cmpSet(ref<Expr> &addr1, ref<Expr> &addr2);
  ref<Expr> cmpTag(ref<Expr> &addr1, ref<Expr> &addr2);

  // used only in determining whether to do specu exec at branch
  // return true -> branch involves no memory access -> no specu exec
  // return false -> do specu exec
  bool simpleAnalyzeMemCache(ExecutionState &state, TimingSolver *solver);

  void analyzeMemCache(ExecutionState &state, TimingSolver *solver);

  // return value only used in simple mode, called by simpleAnalyzeMemCache
  // return true if crt memory access always hits
  // return false is not-defined
  bool analyzeThisAccess(ExecutionState &state, TimingSolver *solver, unsigned crt);
  Solver::Validity isSameAddr(unsigned crt, unsigned pre, 
      ExecutionState &state, TimingSolver *solver, ref<Expr>& uCnstr);
  bool isEvictedLRU(unsigned crt, unsigned pre, unsigned ignoreCnt, 
      ExecutionState& state, TimingSolver* solver, ref<Expr>& uCnstr);

  /********************** Speculative Execution Modeling ***************/

  void stopSpeculativeExecution(ExecutionState &current);
  
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal, ExecutionState*& specu1, ExecutionState*& specu2);

  /*********************** Cache Behavior Analysis **********************/

  std::vector<unsigned> detectedLocation;
	bool detectLeak(ExecutionState& state, ref<Expr>& hitCnstr, TimingSolver* solver);

  /*********************** Debug Util ***********************************/

  void printMemoryAddr(ExecutionState& state);
